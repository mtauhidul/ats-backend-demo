...
export class CandidateFirestoreService extends FirestoreBaseService<FirestoreCandidateData> {
  ...
  async findByEmail(
    email: string
  ): Promise<(FirestoreCandidateData & { id: string }) | null> {
    try {
  ...
  async findByJobId(
    jobId: string
  ): Promise<(FirestoreCandidateData & { id: string })[]> {
    try {
      const filters: QueryFilter[] = [
        { field: 'jobIds', operator: 'array-contains', value: jobId },
      ]
  ...
  async findByStage(
    ...
    try {
      const filters: QueryFilter[] = [
        { field: 'currentStage', operator: '==', value: stage },
      ]
      return await this.find(filters, {
        orderBy: [{ field: 'updatedAt', direction: 'desc' }],
      })
